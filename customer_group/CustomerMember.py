...
class CustomerMember():
    ...
    def commit(self):
        ...
        self.DB[key] = value
        if key == 'create_seller':
            response = self.cust_db.create_seller(value)
        elif key == 'create_buyer':
            response = self.cust_db.create_buyer(value)
        elif key == 'get_buyer_id':
            response = self.cust_db.get_buyer_id(value)
        elif key == 'get_seller_id':
            response = self.cust_db.get_seller_id(value)
        elif key == 'get_seller_rating':
            response = self.cust_db.get_seller_rating(value)
